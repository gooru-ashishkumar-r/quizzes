...
exports.stubby = {
  'Test Places endpoint': function(test) {
    ...
    request('http://localhost:8000')
      ...
      .end(function(err, res) {
        ...
        test.deepEqual(
          res.body,
          require(`${__dirname}/features/response.json`)
        );
        test.done();
      });
  },
  ...
  'Test Localities endpoint': function(test) {
    ...
    request('http://localhost:8000')
      .post(
        '/localities/istambul?exclamation=post requests can have query strings!'
      )
      .set('Content-Type', 'application/xml')
      .send(
        '<!xml blah="blah blah blah"> <envelope>\n   <unaryTag/>\n</envelope>\n'
      )
      .expect(200)
      ...
      .end(function(err, res) {
        ...
        test.equal(
          res.text,
          '<!xml blah="blah blah blah"> <responseXML>\n   <content></content>\n</responseXML>'
        );
        test.done();
      });
  },
