...
module.exports = function(grunt) {
      ...
      if (_.isObject(object.response)) {
        ...
        if (_.isArray(object.response)) {
          ...
          if (
            ...
            !grunt.file.isPathAbsolute(object.response.file)
          ) {
  ...
  grunt.registerMultiTask(
    'stubby',
    'A Grunt plugin for setting up a Stubby server based on YAML/JSON configuration files',
    function() {
              ...
              .map(function(filepath) {
                ...
                if (/.yaml$/g.test(filepath)) {
                  ...
                  try {
                    ...
                    grunt.fail.fatal(
                      `Error while parsing JS file "${filepath}"`,
                      1
                    );
                  }
                } else {
                ...
                return options.relativeFilesPath
                  ? setPathStaticFiles(data, filepath)
                  : data;
              })
          );
      ...
      // start stubby server
      stubbyServer.start(
        _.omit(options, 'callback', 'relativeFilesPath', 'persistent'),
        function(error) {
          if (error) {
          ...
          grunt.log.writeln(
            `Stubby HTTP server listening on port ${options.stubs}`
          );
          grunt.log.writeln(
            `Stubby HTTPS server listening on port ${options.tls}`
          );
